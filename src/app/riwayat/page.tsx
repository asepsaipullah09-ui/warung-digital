          ...
          return <div className="space-y-4">
            ...
            <div className="border rounded-xl overflow-x-auto">
              <div className="min-w-[40rem]">
                <div className="grid grid-cols-6 gap-3 bg-gray-50 px-3 py-2 text-[10px] font-bold text-gray-500 uppercase"><span>Produk</span><span>Stok Sistem</span><span>Fisik</span><span>Terjual</span><span>Pribadi</span><span>Laba</span></div>
                {selectedOpname.items.map(item => <div key={item.id} className="grid grid-cols-6 gap-3 px-3 py-2 border-t text-xs items-center"><span className="font-semibold truncate">{productMap.get(item.product_id)?.name || 'Produk tidak ditemukan'}</span><span>{formatStock(item.product_id, Number(item.system_stock_base))}</span><span>{formatStock(item.product_id, Number(item.physical_stock_base))}</span><span>{formatStock(item.product_id, Number(item.calculated_sales_base))}</span><span>{formatStock(item.product_id, Number(item.personal_use_base))}</span><span className="font-bold text-blue-700">{formatRupiah(Number(item.profit_amount || 0))}</span></div>)}
              </div>
            </div>

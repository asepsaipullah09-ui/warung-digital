...
export function Header() {
  ...
  const refreshStatus = () => {
    const stats = warungStore.getDashboardStats();
    setIsCompleted(stats.is_rekap_completed_today);
    ...
    <>
      <header className="bg-white border-b border-gray-200/70 sticky top-0 z-30 px-4 md:px-8 py-3 flex items-center justify-between gap-4 shadow-2xs">
        ...
        <div className="flex items-center gap-3 ml-auto">
          ...
          {/* Rekap Status Badge (Clean & Single Link to Rekap Malam) */}
          {isCompleted ? (
      ...
      {/* Quick Search Modal */}
      <Modal isOpen={isSearchOpen} onClose={() => setIsSearchOpen(false)} title="Pencarian Cepat Warung">

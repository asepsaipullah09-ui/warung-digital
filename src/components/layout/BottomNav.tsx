    ...
    <>
      ...
      {isMenuOpen && (
        <div className="fixed inset-0 z-50 md:hidden flex flex-col justify-end bg-black/50 backdrop-blur-xs animate-in fade-in duration-200">
          <div className="bg-white rounded-t-2xl p-5 border-t border-gray-100 shadow-2xl max-h-[85vh] overflow-y-auto">
            <div className="flex items-center justify-between pb-4 mb-4 border-b border-gray-100">
              <h3 className="font-bold text-gray-900 text-base">Menu Lainnya</h3>
              <button
              ...
              {moreItems.map((item) => {
                ...
                return (
                  <Link
                    ...
                    onClick={() => setIsMenuOpen(false)}
                    className={cn(
                      'flex items-center gap-3.5 p-3.5 rounded-xl transition-all',
                      isActive ? 'bg-emerald-50 text-emerald-700 font-semibold' : 'bg-gray-50 text-gray-700 hover:bg-gray-100'
                    )}
                  >
                    <div className={cn('p-2 rounded-lg', isActive ? 'bg-emerald-600 text-white' : 'bg-white text-gray-600 shadow-xs')}>
      ...
      {/* Fixed Bottom Bar */}
      <div className="md:hidden fixed bottom-0 left-0 right-0 z-40 bg-white border-t border-gray-200 px-3 py-1.5 flex items-center justify-around shadow-lg">
        {primaryItems.map((item) => {
          ...
          if (item.isPrimaryAction) {
            return (
              ...
              >
                <div className="w-13 h-13 rounded-full bg-emerald-600 text-white flex items-center justify-center shadow-lg shadow-emerald-600/40 ring-4 ring-white active:scale-95 transition-transform">
                  <Moon className="w-6 h-6 fill-emerald-600" />
                </div>
                <span className="text-[11px] font-bold text-emerald-700 mt-0.5">Rekap</span>
              </Link>
            );
          }
          ...
          return (
            ...
            >
              <Icon className={cn('w-5 h-5 mb-0.5', isActive ? 'text-emerald-600' : 'text-gray-400')} />
        ...
        <button
          ...
          className={cn(
            'flex flex-col items-center py-1 px-3 rounded-lg text-xs font-medium transition-colors',
            isMenuOpen ? 'text-emerald-600 font-bold' : 'text-gray-500 hover:text-gray-900'
          )}
        >
